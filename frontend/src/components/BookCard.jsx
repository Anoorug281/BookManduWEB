...
const BookCard = ({data,favourite}) => {
    ...
    const handleRemoveBook = async()=>{
        const response = await axios.put(
            "http://localhost:1000/api/user/remove-book-from-notification",
            {},
            {headers}
        )
  ...
  return (
    <>
        <div className='bg-zinc-800 rounded p-4 flex flex-col'>
            ...
            {favourite && (
                <button className='bg-yellow-100 px-4 py-2 rounded border border-yellow-500 text-yellow-500 mt-4' onClick={handleRemoveBook}>
